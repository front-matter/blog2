---
layout: post
title:  "BlueObelisk StackExchange: summary of the first month"
date:   2009-12-21
modified_date: 2026-03-29
blogger-link: https://chem-bla-ics.blogspot.com/2009/12/blueobelisk-stackexchange-summary-of.html
doi: 10.59350/x958j-0xm21
tags:
---

The [Blue Obelisk StackExchange <i class="fa-solid fa-box-archive fa-xs"></i>](https://web.archive.org/web/20091231032042/http://blueobelisk.stackexchange.com/)
(BO<sub>x</sub>) has seen a relatively good start,
but the number of questions is dropping. The average number of unique visits is about 23-30 now:

![](/assets/images/box.png)

The number of [registered users <i class="fa-solid fa-box-archive fa-xs"></i>](https://web.archive.org/web/20091231024627/http://blueobelisk.stackexchange.com/users) is not insignificant but also
has not been growing much lately:

![](/assets/images/box1.png)

At the same time, the quality of the questions are high, and have real users questions:

* [Good way to produce a table of R-groups <i class="fa-solid fa-link-slash fa-xs"></i>](http://blueobelisk.stackexchange.com/questions/86/good-way-to-produce-a-table-of-r-groups) (yet unanswered!)
* [Is there an open source pKa or LogD tool available <i class="fa-solid fa-link-slash fa-xs"></i>](http://blueobelisk.stackexchange.com/questions/91/is-there-an-open-source-pka-or-logd-tool-available)
* [Easiest way to align two molecules from the command line? <i class="fa-solid fa-link-slash fa-xs"></i>](http://blueobelisk.stackexchange.com/questions/81/easiest-way-to-align-two-molecules-from-the-command-line)

The overall state is [37 questions <i class="fa-solid fa-box-archive fa-xs"></i>](https://web.archive.org/web/20100413233403/http://blueobelisk.stackexchange.com/questions) with about
[50 different tags <i class="fa-solid fa-link-slash fa-xs"></i>](http://blueobelisk.stackexchange.com/tags):

![](/assets/images/box2.png)

To the make progress with BO<sub>x</sub>, we primarily need to promote it more as central point of entry to people who
want to know what *free* tools they can use to perform there need, and to the people who want to contribute to
*ODOSOS cheminformatics*, by pointing out the unsolved problems.
